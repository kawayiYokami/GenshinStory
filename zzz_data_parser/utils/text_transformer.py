...
def transform_text(
    ...
    processed_text = re.sub(r'\(test\)|（test）|\(废弃\)|（废弃）', '', processed_text, flags=re.S)
    ...
    if gender.upper() == 'M':
        processed_text = re.sub(r'\{M#(.*?)\}', r'\1', processed_text, flags=re.S)
        processed_text = re.sub(r'\{F#.*?\}', '', processed_text, flags=re.S)
        mate_name = '玲'
    else: # 默认为女性
        processed_text = re.sub(r'\{F#(.*?)\}', r'\1', processed_text, flags=re.S)
        processed_text = re.sub(r'\{M#.*?\}', '', processed_text, flags=re.S)
        mate_name = '哲'

    # 步骤 4: 应用 placeholder_map.py 中定义的所有通用正则表达式规则
    for pattern, replacement in REGEX_RULES:
        processed_text = re.sub(pattern, replacement, processed_text, flags=re.S)
    ...
    processed_text = re.sub(r'\{(.*?)\}', r'\1', processed_text, flags=re.S)

"""
Module for parsing raw message data into structured Session objects.
This parser implements the logic based on the analysis of data fields:
...
class SessionParser:
    """
    Parses raw message data dictionaries into structured Session objects.
    ...
    def __init__(self, text_map: Dict[str, str], npc_config: Optional[Dict[str, Any]] = None):
        """
        Initializes the parser with a text map for resolving text keys and optionally NPC config.
    ...
    def _get_npc_name_by_id(self, npc_id: int) -> str:
        ...
        """
        if not self.npc_config or not npc_id:
            return "[未知NPC]"

        # Find NPC data by ID (CJHNGFMGKGE is the ID field in NPC config)
        npc_data = None
        for item in self.npc_config.get("PDHBFPILAJD", []):
            if item.get("CJHNGFMGKGE") == npc_id:
                npc_data = item
                break

        if not npc_data:
            return f"[未知NPC ID: {npc_id}]"

        # Get the name key (LNCODBGHFKM is the name key field in NPC config)
        name_key = npc_data.get("LNCODBGHFKM")
        if not name_key:
            return f"[NPC名称键缺失 ID: {npc_id}]"

        # Resolve the name key using the text map
        return self.text_map.get(name_key, f"[NPC名称未找到: {name_key}]")

    def parse(self, raw_messages: List[Dict]) -> List[Session]:
        """
        Parses a list of raw message dictionaries into structured Session objects.
        ...
        for session_id, messages in sessions_data.items():
            session = self._parse_single_session(session_id, messages)
            sessions.append(session)

        return sessions

    def _parse_single_session(self, session_id: int, raw_messages: List[Dict]) -> Session:
        """
        Parses messages for a single session.
        """
        ...
        for sequence_id, stage_messages in stages_data.items():
            stage = self._parse_stage(sequence_id, stage_messages)
            session.stages[sequence_id] = stage

        return session

    def _parse_stage(self, sequence_id: int, raw_messages: List[Dict]) -> Stage:
        """
        Parses messages for a single stage based on their type.
        """
    ...
    def _create_message(self, raw_msg: Dict) -> Message:
        ...
        return Message(
            ...
            text=text
            # unlocks_condition is handled in _parse_stage
        )

import tokenizer from './tokenizerService';
...
class ContextOptimizerService {
    ...
    public async processContext(history: Message[], maxTokens: number): Promise<OptimizationResult> {
        ...
        logger.log(`[优化器] 分离完成。早期历史: ${earlyHistory.length}条, 近期历史: ${recentHistory.length}条。`);

        if (!earlyHistory || earlyHistory.length === 0) {
            logger.warn('[优化器] 最近的对话本身已超出 Token 限制，无法压缩。');
            return {
        ...
        let summary: string;
        try {
            logger.log('[优化器] 准备摘要的早期历史内容:', earlyHistory);
        ...
        const newHistory = [systemPrompt, summaryMessage, ...recentHistory].filter(Boolean) as Message[];

  ...
  async execute(params: AskParams): Promise<ToolExecutionResult> {
    try {
      ...
      if (!question || suggestions.length < 2) {
        return {
          result: "错误: 参数不完整。'ask_choice' 工具需要明确的 'question' 和至少2个 'suggest' 选项。"
        };
      }

      // ask_choice 工具不需要后端执行，直接返回用户提问格式
      ...
      logger.error(`工具 ask_choice 发生意外异常:`, error);
      return {
        result: `错误: 工具 'ask_choice' 内部执行失败。请检查参数是否符合工具用法，或尝试其他方法。`
      };
    }
  }
};

...
def get_physical_path(item):
    ...
    domain_name = "gi"  # Hardcoded for GiWiki script

    # Example frontendPath: /v2/gi/category/角色逸闻/那维莱特【逸闻】-506130
    parts = frontend_path.split('/')
    if len(parts) < 5:
        return None

    # parts[0] is empty, [1] is 'v2', [2] is 'gi', [3] is 'category'
    relative_path = "/".join(parts[4:])
    return f"/domains/{domain_name}/docs/{relative_path}.md"

def create_catalog_tree():
    """
    从主索引文件 (index.json) 创建一个层级的、类似文件系统的目录树。
    """
